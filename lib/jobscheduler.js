...
var checkEnd = function() {
  if (totalCount !== 0 && childrenCount === 0 && totalCount == (successCount + failureCount)) {
...
var statsServer = function() {
  server = net.createServer(function(con) {
    ...
    con.write('concurrency: ' + childrenCount + '\r\n');
    con.pipe(con);
  });
...
urls.then(function(urls) {
  ...
  var exitHandler = function() {
    ...
    if (queue.length > 0 && childrenCount < config.concurrency) {
  ...
  options.forEach(function(opt) {
    ...
    if (childrenCount < config.concurrency) {
      childrenCount += 1;
      var child = cp.fork(__dirname + '/subjob.js', [JSON.stringify(opt)]);
      child.on('exit', exitHandler);
      child.on('message', messageHandler);
